...
/* Author: Divya Saxena                                                      */
/* Date: 2 Mar 2020                                                          */
/* Modifications :  Q learning    Algo  with eligibilty Traces               */
/*****************************************************************************/
...
#define STATES 81
#define ACTIONS 2
#define EPSILON .001//dsx
#define LAMDA 0.1//dsx
#define ALPHA 0.1
#define GAMMA 0.95

long double Q[STATES][ACTIONS];
long double E[STATES][ACTIONS];//dsx eligibility matrix
...
// dsx Q[prevstate][prevaction] += ALPHA*((long double)reward(*fail) + GAMMA * maxofQ(currstate) - Q[prevstate][prevaction]);
...
if ((*fail == 0)){
  curraction = get_Qaction(invert,currstate,force,explore);
}
else{
  curraction =prevaction;
}
	     a_star = action_star(currstate);
	
	     delta = ((long double)reward(*fail) + GAMMA * Q[currstate][a_star] - Q[prevstate][prevaction]);
	     E[prevstate][prevaction] += 1;
	
	     for (i = 0; i < STATES; i++){
	         for (j = 0; j < ACTIONS; j++) {
	            Q[i][j] += delta * ALPHA * E[i][j];
	            E[i][j] = (curraction == a_star) ? (GAMMA * LAMDA * E[i][j]) : 0.0;
