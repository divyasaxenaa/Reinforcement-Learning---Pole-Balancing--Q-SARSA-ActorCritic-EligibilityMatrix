...
/* Author: Divya Saxena                                                      */
/* Date: 2 Mar 2020                                                          */
/* Modifications :   SARSA    with eligibilty Traces                         */
/*****************************************************************************/
...
#define ACTIONS 2
#define EPSILON .001
#define LAMDA 0.9//dsx
#define ALPHA 0.1
#define GAMMA 0.95

long double Q[STATES][ACTIONS];
	long double E[STATES][ACTIONS];//dsx
...
if (resetflag == 0){ // Here we have passed the initial state and we can update the Q
      ...
      if (*fail == 0){
       ...
       else
       {
	     ...
	     //dsx Q[prevstate][prevaction] += ALPHA*((long double)reward(*fail) + GAMMA * Q[currstate][curraction] - Q[prevstate][prevaction]);

       //dsx start
        delta = ((long double)reward(*fail) + GAMMA * Q[currstate][curraction] - Q[prevstate][prevaction]);
