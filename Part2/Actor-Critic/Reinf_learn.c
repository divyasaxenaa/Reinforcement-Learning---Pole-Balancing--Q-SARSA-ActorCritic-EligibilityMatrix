...
/* Author: Divya Saxena                                                      */
/* Date: 2 Mar 2020                                                          */
/* Modifications :   Actor Critic Algo  with eligibilty Traces               */
/*****************************************************************************/
...
long double e_actor[states][actions];//dsx
	long double Q[states][actions];//dsx
	long double e_critic[states];//dsx
...
//Start By Divya
	...
	// updating Q and eligibility
      ...
      //dsx start
  delta =  (long double)reward(*fail) + gamma *  value[currstate] - value[prevstate];
	     e_critic[prevstate] += 1;
	     e_actor[prevstate][prevaction] += 1;
	    ...
	    for (i = 0; i < STATES; i++){
	        ...
	        for (j = 0; j < actions; j++) {
	            prefstateaction[i][j] +=  alpha * delta * e_actor[i][j];
